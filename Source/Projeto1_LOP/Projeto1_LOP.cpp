...
class ls_engine {
	...
	double lim_fail_after_imp_factor = -1;

public:
	bool print_inside_ls = false;
...
public:	
	...
	void config(ls_algorithms_option ls_algorithm , move_strg mv_strg, 
		...
		bool do_second_order_srch_ = false, double _lim_fail_after_imp = -1) {
		...
		lim_fail_after_imp_factor = _lim_fail_after_imp;
	}
	...
	string signature(char sep = ' ' , bool omit_roud_order = false) {
		...
		if (lim_fail_after_imp_factor > 0) ret += "(max_fail:" + to_string(lim_fail_after_imp_factor) + ")";
	...
	void ls_array_insert(solution_1& s) {
		...
		do {
			...
			for (int vv = 0; vv < n; vv++) {
				...
				if (lim_fail_after_imp_factor > 0 && fail_after_imp >= lim_fail_after_imp_factor*n) break;
			}
			...
			if (best_v == -1 || best_improve <= 0) {
				...
				if (print_inside_ls)
				{
					cout << v << " " << u << " " << (abs(s.pi_inv[v] - s.pi_inv[u]))
						<< " " << best_improve;
				}
			...
			if (print_inside_ls) {
				auto this_roud_time = chrono::duration_cast<chrono::nanoseconds>(end_round_time - start_round_time).count();
				cout << " " << to_string(this_roud_time) << endl;
			}
...
ls_engine::round_order_strg, bool, bool, double > ls_conf_type;
...
class mh_engine {
	...
	vector<int> best_pi;
	long best_fo = -1;
	int n_iter_no_improve;
	//control
	int iter_no_improve_limit = 30,
		iter_no_improve_change_strg_limit = iter_no_improve_limit*0.8;
	
	double n_swap_factor = 0.05; //5%

	void init_mem() {
		n_iter_no_improve = 0; best_fo = -1;
		best_pi.clear();
		n_swap_factor = 0.02;
	}

	bool is_rand_mode() { return n_iter_no_improve >= iter_no_improve_change_strg_limit; }

public:
	void config(ls_engine &_ls_engine) {
		my_ls = &_ls_engine;
	}

	void run_ils(solution_1 &s) {
		bool stop_mh = false;
		init_mem();
		...
		do {
			...
			my_ls->run_ls(s);
			//analyse local minimum
			lm_analysis(s);
			//test stop criterion
			if (test_stop()) stop_mh = true;

			if (!stop_mh) {
				//choose base solution
				choose_next(s);

				//shake base solution to generate next starting point
				if(!is_rand_mode())
					shake(s);
			}
		} while (!stop_mh);
	...
	long get_best_fo() { return best_fo; }

	vector<int> get_best_pi() { return best_pi; }

private:

	void lm_analysis(solution_1 &s) {
		auto act_fo = s.get_fo();
		if (best_fo == -1 || act_fo > best_fo) {
	...
	bool test_stop(){
		if (n_iter_no_improve >= iter_no_improve_limit)
			return true;
		else
			return false;
	}

	void choose_next(solution_1 &s) {
		if (n_iter_no_improve >= iter_no_improve_change_strg_limit) {
			//choose random start point
			vector<int> random_pi(s.n);
			for (int i = 0; i < s.n; i++)random_pi[i] = i;
			shuffle(random_pi.begin(), random_pi.end(), rand_gen);
			s.reset_pi(random_pi);
		}
		else{
			//choose best solution found			
			if (s.get_fo() == best_fo) return;
			s.reset_pi(best_pi);
		}
	}

	void shake(solution_1 &s) {
		int n_swaps = s.n * n_swap_factor;
		if (n_iter_no_improve >= 0.5*iter_no_improve_limit)
			n_swaps *= 2;

		vector<int> new_pi = s.pi;
		for (int i = 0; i < n_swaps; i++) {
			int pv = rand_gen() % s.n;
			int pu = rand_gen() % (s.n - 1);
			if (pu >= pv) pu++;

			swap(new_pi[pv], new_pi[pu]);
		}
		s.reset_pi(new_pi);
	}
...
public:
	string insts_folder_path = "C:\\Users\\Lucas\\Estudos\\PucRio\\Metaheuristicas\\Projeto1\\";
	...
	*/
	...
	while (true)
	{
		...
		for (auto ls_config : ls_configs) {
			...
			if (print_ls_history) {
				//open especific output files
				case_out.open(case_signature + ".txt");
			...
			/*
				At this point the ls´s of a sprint have been run.
				We need to print aggregate data for theses local minimums
			*/

			//======= print data line ===========
			string srmr_duration, snumber_of_runs, sfo_max, sfo_mean, sfo_sd, 
			sn_moves_mean, sn_moves_sd, smean_roud_time;
			...
			//n d i ls_name move_strg rmr_duration number_of_runs fo_max fo_mean fo_sd n_moves_mean n_moves_sd n_moves2_mean mean2_round_time
			cout << case_signature 
				<< sep << srmr_duration << sep << snumber_of_runs 
				<< sep << sfo_max << sep << sfo_mean << sep << sfo_sd 
				<< sep << sn_moves_mean << sep << sn_moves_sd
				<< sep << smean_roud_time
			...
			// do hard_check on best solution found
...
// ==== Metaheuristics Experiemnts ======

void do_experiment_mh(
	vector<int>& n_sizes, vector<int>& dens_sizes, vector<int>& inst_i_sizes
) {
	
	
	string exp_name = "Metaheiristics Experiment";
	cout << "!" << exp_name << " ";
	cout << endl;
	...
	*/
	...
	while (true)
	{
		...
		solution_1 s1;
		ls_engine ls;
		ls.config(ls_engine::ls_algorithms_option::bs_array_alg, 
			ls_engine::move_strg::first,
			ls_engine::round_order_strg::rand_round_order,
			false,
			false,
		...
		mh_engine ils_engine;
		ils_engine.config(ls);
		...
		s1.init(inst, inst.generate_random_pi(),
			ls.my_algorithm == ls_engine::ls_algorithms_option::basic_alg);
		
		auto start_mh_time = get_time::now();
		ils_engine.run_ils(s1);
		auto finish_time = get_time::now();

		auto fo = ils_engine.get_best_fo();
		auto time_sec = to_time(finish_time - start_mh_time) / 1000.0;
		cout << case_signature
			<< sep << fo 
			<< sep << time_sec << endl;
		...
		// ==== end of rmr analysis for a ls_config ====
		// do hard_check on best solution found
		inst.hard_check(ils_engine.get_best_pi(), fo);
...
{	
	...
	auto array_best_rand = make_tuple(
		ls_engine::ls_algorithms_option::array_alg,
		ls_engine::move_strg::best,
		ls_engine::round_order_strg::rand_round_order,
		false,
		false,
	...
	auto bs_first_fixed = make_tuple(
		ls_engine::ls_algorithms_option::bs_array_alg,
		ls_engine::move_strg::first,
		ls_engine::round_order_strg::idx_order,
		false,
	...
	auto tree_best_rand = make_tuple(
	...
	auto tree_bestheap_rand = make_tuple(
		ls_engine::ls_algorithms_option::tree_alg,
	...
	auto tree_first_rand = make_tuple(
		ls_engine::ls_algorithms_option::tree_alg,
		ls_engine::move_strg::first,
		ls_engine::round_order_strg::rand_round_order,
		false,
	...
	auto basic_first_rand = make_tuple(
	...
	auto basic_2ord_first_rand = make_tuple(
		ls_engine::ls_algorithms_option::basic_alg,
		ls_engine::move_strg::first,
		ls_engine::round_order_strg::rand_round_order,
		false, // use memory?
	...
	auto basic_2ord_best_rand = make_tuple(
		ls_engine::ls_algorithms_option::basic_alg,
		ls_engine::move_strg::best,
		ls_engine::round_order_strg::rand_round_order,
		false, // use memory?
	...
	//ls_configs.push_back(array_best_rand);
	ls_configs.push_back(bs_best_rand);
	ls_configs.push_back(basic_2ord_best_rand);
	//ls_configs.push_back(basic_2ord_first_rand);
	//ls_configs.push_back(bs_first_rand);

	//ls_configs.push_back(tree_first_rand);	
	
	//ls_configs.push_back(basic_first_rand);
	
	/*
	do_experiment_multi_restart(ls_configs, n_sizes, dens_sizes, inst_i_sizes,
		800, //time limit
		100, //max runs
		stop_criterion_type::time_limit //stop criterion type
	);*/

	do_experiment_mh(n_sizes, dens_sizes, inst_i_sizes);
